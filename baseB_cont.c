...
int main(int argc, const char * argv[]) {
    ...
    double variance_gaussian = atof(argv[1]);
    double variance_e = atof(argv[2]);
    
    double BIRTH_RATE_M1_E1=0.95 + variance_e;
    
    double BIRTH_RATE_M2_E1=0.95 - variance_e;

    // gsl_vector * mu = gsl_vector_calloc(N_0);
    // gsl_matrix * Sigma = gsl_matrix_calloc(N_0, N_0);
    // gsl_vector_set_all(mu, BIRTH_RATE_W_E1)
    
    double p=atof(argv[3]); //memory, eventually as argument
    ...
    for (run=0; run<NRRUNS; run++){
        ...
        double M1_birth_rates[K]; //Initialize variable for mutant population birth rates tracking
        int M2_Size = 0; //Initialize variable for mutant population size tracking
        double M2_birth_rates[K]; //Initialize variable for mutant population birth rates tracking
        ...
        double B_rate_M1=sum(M1_birth_rates, M1_Size); //*(1-N/K);
        double B_rate_M2=sum(M2_birth_rates, M2_Size); //*(1-N/K);
        ...
        while (N>0 ){
            ...
            if (theEventNr==0 & W_Size>0){ //B_rate_A
                ...
                if (mutate==1 ){ //1 = mutates;
                    ...
                    double gaussian_sample = gsl_ran_gaussian(gBaseRand, variance_gaussian)+BIRTH_RATE_M1_E1;
                    if (gaussian_sample < 0){
                        M2_birth_rates[M2_Size] = 0;
                    }
                    else{
                        M2_birth_rates[M2_Size] = gaussian_sample;
                    }
                    M2_Size++;
                ...
                if (theEnv==1){
                    ...
                    B_rate_M1=sum(M1_birth_rates, M1_Size); //*(1-N/K);
                    B_rate_M2=sum(M2_birth_rates, M2_Size); //*(1-N/K);
                ...
                if (tokeep==0){ //0 = redraw phenotype.
                    ...
                    if (which==0){
                        double gaussian_sample = gsl_ran_gaussian(gBaseRand, variance_gaussian)+BIRTH_RATE_M1_E1;
                        if (gaussian_sample < 0){
                            M1_birth_rates[M1_Size] = 0;
                        }
                        else{
                            M1_birth_rates[M1_Size] = gaussian_sample;
                        }
                        M1_Size++;
                    } else {
                        double gaussian_sample = gsl_ran_gaussian(gBaseRand, variance_gaussian)+BIRTH_RATE_M2_E1;
                        if (gaussian_sample < 0){
                            M2_birth_rates[M2_Size] = 0;
                        }
                        else{
                ...
                NR_Deaths++;
                unsigned  int which_die1=gsl_rng_uniform_int(gBaseRand,M1_Size);
                M1_birth_rates[which_die1] = M1_birth_rates[M1_Size-1];
                M1_Size--;
                M_Size=M1_Size+M2_Size;
                N=W_Size+M1_Size+M2_Size;
                if (theEnv==1){
                ...
                NR_Deaths++;
                unsigned  int which_die2=gsl_rng_uniform_int(gBaseRand,M2_Size);
                M2_birth_rates[which_die2] = M2_birth_rates[M2_Size-1];
                M2_Size--;
                M_Size=M1_Size+M2_Size;
                N=W_Size+M1_Size+M2_Size;
                if (theEnv==1){
    ...
    char filename[500];
    sprintf(filename, "log/fig2B_cont/outfileB_K_%d_memory_%f_BWE1_%f_BM1E1_%f_BM2E1_%f_mutationAa_%f_std%f_N1000.txt", K, p, BIRTH_RATE_W_E1, BIRTH_RATE_M1_E1, BIRTH_RATE_M2_E1, mutationAa, variance_gaussian);
